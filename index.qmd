...
1) First, we import the dataset that contains basically the surnames, their country of origin and pronoun, as well as the year we started working together. 

2) Using `colorRampPalette()`, we extend the colors from basic colours present in high contrast points of Lygia Clark's painting "[Composição (1953)](https://portal.lygiaclark.org.br/en/artwork/55666/composition)".

3) Finally, we generate one rectangle for each colleague with the side proportional to the time since we met. Colours represent the combination of pronoun and country of origin, and transparency is proportional to size.
...
# Generating one color for each group in the dataset
compo <- colorRampPalette(lc_palette)(length(unique(df$group)))


# 3) Generating the plot #####################################

# Adding empty columns for coordinates
df <- df |> mutate(x1 = NA, x2 = NA, y1 = NA, y2 = NA)
...
for (i in sample(1:n_colab)){
  ...
  # If pronoun equals to s then go right and up otherwise the opposite
...
ggsave(plot_compo, filename = "./img/composicao.png", width = 8, height = 4)

plot_compo + 
  theme(
    panel.border = element_rect(colour = "grey40", fill=NA, linewidth=2),
    plot.background = element_rect(fill = lc_canvas)
  )
...
![Composicao (1953) - Lygia Clark (Source: [Associação Cultural Lygia Clark](https://portal.lygiaclark.org.br/))](img/lygiaclark_compo.jpg){width=400}
...
ggsave(plot_time, filename = "./img/timeline.png")
...
ggsave(uis_map, filename = "./img/uis_map.png", dpi = 320, width = 6, height = 5, bg = "transparent")
...
:::{.aside}
![Figure 16 of the SDG4 Data Digest 2019. Source: [UIS](https://unesdoc.unesco.org/ark:/48223/pf0000372591.locale=en)](./img/uis_publication.png)
:::
